"""Serve a run's raw volume and masks together in neuroglancer.

A raw .tif isn't zarr/n5/precomputed-backed, so it has no fileglancer URL
neuroglancer can fetch directly -- this loads it in-process (same load_volume
as run_timepoint.py) and serves both layers via neuroglancer.LocalVolume.
Keeps this process alive (and its server) until you press enter.

Usage:
    uv run --extra view scripts/serve_view.py <config.toml> <run-slug>
    uv run --extra view scripts/serve_view.py <run_dir>
"""
...
# run_timepoint.py is a standalone script, not an importable package, so
# its load_volume is loaded directly from its file path here.
_spec = importlib.util.spec_from_file_location(
    "run_timepoint", Path(__file__).parent / "run_timepoint.py"
)
