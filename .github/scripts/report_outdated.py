"""Report dependencies whose newest release is excluded by its declared bound.
...
TREE_LINE = re.compile(r"^\s*[^a-zA-Z0-9]*([A-Za-z0-9._-]+)\s+v(\S+)\s+\(latest:\s+v(\S+)\)")
...
def outdated_entries() -> list[tuple[str, str, str]]:
    ...
    result = subprocess.run(
        ["uv", "tree", "--outdated", "--depth", "1", "--no-dev"],
        capture_output=True,
        text=True,
        check=True,
